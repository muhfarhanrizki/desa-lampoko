    ...
    <section class="max-w-6xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-3 gap-8 pt-36 pb-16">
        ...
        <div class="md:col-span-2 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <button onclick="history.back()" 
                class="mb-5 inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 transition text-sm">
            ...
            <h1 class="text-3xl font-semibold text-gray-800 mb-3">{{ $artikel->judul }}</h1>
            <p class="text-sm text-gray-500 italic flex items-center gap-2 mb-2">
            ...
            <p class="text-sm text-gray-500 mb-3 flex items-center gap-2 ">
                ...
                Oleh <span class="font-medium text-gray-700">{{ $artikel->penulis ?? 'Admin' }}</span>
            </p>
        ...
        <aside class="md:col-span-1 space-y-5">
            <h2 class="text-2xl font-semibold mb-6 border-b border-gray-200 pb-2 text-blue-700">Artikel Terbaru</h2>
            
            @foreach($relatedArtikels as $related)
                <a href="{{ route('artikel.informasi.show', $related->slug) }}" 
                   class="block bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg hover:scale-[1.02] transition-transform duration-300 p-4">
                    ...
                    <div class="flex items-center gap-4">
                        <img src="{{ asset('uploads/artikel/' . $related->gambar) }}" 
                            onerror="this.onerror=null;this.src='{{ asset('images/no-image.png') }}';"
                            alt="{{ $related->judul }}" 
                        ...
                        <div class="flex flex-col">
                            ...
                            <p class="text-xs text-blue-600 mt-1 flex items-center gap-1 font-semibold">
