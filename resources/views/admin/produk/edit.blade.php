        ...
        <form action="{{ route('produk.update', $produk->slug) }}" method="POST" enctype="multipart/form-data"
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                          class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none">{{ old('deskripsi', $produk->deskripsi) }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" name="alamat" id="alamat" value="{{ old('alamat', $produk->alamat) }}" required
                       class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            ...
            <div>
                ...
                <input type="text" name="kontak" id="kontak" value="{{ old('kontak', $produk->kontak) }}" required
                       class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>

                <div>
                <label for="penjual" class="block text-sm font-medium text-gray-700 mb-1">Nama Penjual</label>
                <input type="text" name="penjual" id="penjual" value="{{ old('penjual', $produk->penjual) }}" required
                       class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            </div>
            ...
            <div>
                ...
                <input type="number" name="harga" id="harga" value="{{ old('harga', $produk->harga) }}"
                    required
                    class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                    placeholder="Contoh: 50000 (tanpa titik/koma)" min="0" step="1000" />
            ...
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    <option value="approved" {{ old('status', $produk->status) == 'approved' ? 'selected' : '' }}>Approved</option>
                </select>
            </div>
