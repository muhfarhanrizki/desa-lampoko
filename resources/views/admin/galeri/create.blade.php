<x-app-layout>
    ...
    <div class="py-6 px-4 max-w-2xl mx-auto">
        ...
        <form action="{{ route('galeri.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-md shadow-md">
            @csrf
            ...
            <div>
                <label for="deskripsi" class="block font-medium text-gray-700 mb-1">Deskripsi Galeri</label>
                <textarea name="deskripsi" id="deskripsi" rows="8" required
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    placeholder="Tulis deskripsi galeri di sini...">{{ old('deskripsi') }}</textarea>
            </div>
