<x-app-layout>
    ...
    <div class="py-6 px-4 max-w-4xl mx-auto">
        ...
        <form action="{{ route('artikel.update', $artikel->slug) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-md shadow-md">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="isi" id="isi" rows="6" required
                    class="w-full border border-gray-300 rounded-md px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none">{{ old('isi', $artikel->isi) }}</textarea>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Gambar Saat Ini</label>
                @if ($artikel->gambar)
                    <img src="{{ asset('uploads/artikel/' . $artikel->gambar) }}" alt="{{ $artikel->judul }}"
